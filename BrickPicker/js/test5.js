...
(function(){
  var express, path, request, async, crypto, fs, fetch, fetchStockData, formatStockData, Close, MA, RSV, KD, reductions, map2, YuMA, EMA, MACDDIF, MACDDEM, checkSignal, checkEarn, app;
  ...
  */
  ...
  map2 = function(f, vs1, vs2){
    var maxLength, i$, ref$, len$, i, results$ = [];
    maxLength = Math.max(vs1.length, vs2.length);
    for (i$ = 0, len$ = (ref$ = (fn$())).length; i$ < len$; ++i$) {
      i = ref$[i$];
      results$.push(f(i < vs1.length
        ? vs1[i]
        : vs1[vs1.length - 1], i < vs2.length
        ? vs2[i]
        : vs2[vs2.length - 1]));
    }
    return results$;
  ...
  */
  ...
  app.get('/view/stock/:year/:cnt/:stockId', function(req, res){
    ...
    return fetchStockData(stockId, [year], (function(){
      ...
      var stockData, close, ma5, ma10, kd, ema5, ema10, dif, dem, i$, ref$, len$, ref1$, line1, line2, e;
      if (err) {
        return res.json(err);
      }
      try {
        ...
        ma5 = MA(5, close);
        ma10 = MA(10, close);
        kd = KD(RSV(9, stockData));
        ema5 = EMA(5, close);
        ema10 = EMA(10, close);
        dif = MACDDIF(12, 26, close);
        dem = MACDDEM(9, dif);
        for (i$ = 0, len$ = (ref$ = [[ma5, ma10], [kd[0], kd[1]], [ema5, ema10], [dif, dem]]).length; i$ < len$; ++i$) {
          ref1$ = ref$[i$], line1 = ref1$[0], line2 = ref1$[1];
          console.log(
          checkEarn(
