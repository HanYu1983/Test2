...
namespace HanUtil
{
	public class HandleIAP : MonoBehaviour
	{
		...
		public event Action<string> OnReceviceResponse = delegate{};
		...
		public void IOSRequestProductData (List<string> skus)
		{
			var skusQuery = string.Join ("&", skus.ToArray());
		...
		public void AndroidGetSkuDetails (List<string> skus)
		{
			var skusQuery = string.Join ("&", skus.ToArray());
			...
			case "IAB.getPurchasesResult":
				{
					try {
						...
						var result = JsonUtility.FromJson<AndroidPurchasesResult>(jsonstr);
			...
			case "IAB.onActivityResult":
				{
					try {
						...
						OnAndroidActivityResult (resultCode, JsonUtility.FromJson<ActivityResult>(data));
					} catch (Exception e) {
						OnAndroidException (e);
					}
				}
