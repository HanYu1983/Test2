...
public class ZUIManager : MonoBehaviour {
    ...
    [Tooltip("Should Escape button act as back button?")]
    ...
    void Start()
    {
        ...
        foreach (Menu m in AllMenus)
        {
            ...
            if (m == CurActiveMenu)
            {
                ...
                {
                    m.ChangeVisibilityImmediate(false);
    ...
    void Update()
    {
        if (EscIsBack && Input.GetKeyDown(KeyCode.Escape))
        {
            Back();
        }
    }
    ...
    /// <summary>
    /// Open current menu's "PreviousMenu".
    /// </summary>
    public void Back()
    {
        ...
        else if (CurActiveMenu.PreviousMenu)
        {
            ...
            OpenMenu(CurActiveMenu.PreviousMenu.name);
        }
    }
